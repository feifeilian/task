...
var _taskdata = __webpack_require__(2);
...
var edit_id = -1;
var edit_text = 'this is a new task';
var edit_status = 'will';
var edit_priority = 'low';
var data = {};
...
window.onload = function () {
    ...
    var getData = taskStorage.getItem("taskdata");

    //如果seesionStorage没有缓存，则缓存数据
    if (!getData) {
        //将json数据转化为字符串
        var d = JSON.stringify(_taskdata2.default);
    ...
    var oneThing = new Hammer(getById("onething"));
    var allThing = new Hammer(getById("allthing"));
    var list = new Hammer(getByClass("list"));
    var add = new Hammer(getByClass('add'));
    var complete = new Hammer(getByClass('complete'));
    var cancel = new Hammer(getByClass('btn-cancel'));
    var choose = new Hammer(getByClass('choose'));

    //添加事件
    oneThing.on("tap", function () {
        return changeList('one');
    });
    allThing.on("tap", function () {
    ...
    list.on("swipeleft", function () {
        return swift(event, 'operate');
    });
    list.on("swiperight", function () {
        return swift(event, 'status');
    });
};
...
function removeClass(ele, clsName) {
    var clsList = ele.className.split(' ');
    for (var i = 0; i < clsList.length; i++) {
        if (clsList[i].trim() == clsName) {
            clsList.splice(i, 1);
        }
    }
    var name = clsList.join(" ");
    ele.className = name;
}
...
function toEdit() {
    ...
    var itemsPriotity = getByClass('priority').getElementsByTagName('div');
...
function operateList(event) {
    var statusIcon = event.target.parentNode.parentNode.getElementsByTagName("span")[0];
...
function chooseTag(e) {
    if (e.target && hasClass(e.target, 'priority-item')) {
        var items = document.getElementsByClassName('priority-item');
...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../node_modules/css-loader/index.js!../node_modules/less-loader/dist/cjs.js!./todolist.less", function() {
			var newContent = require("!!../node_modules/css-loader/index.js!../node_modules/less-loader/dist/cjs.js!./todolist.less");
